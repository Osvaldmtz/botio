-- Exclude trial onboarding / follow-up automation from engagement metrics
-- (message_count, last_message_at, needs_reply in conversation_summary).

CREATE OR REPLACE FUNCTION public.is_automated_metric_message(
  p_role text,
  p_source text,
  p_metadata jsonb
) RETURNS boolean
...
AS $$
  SELECT
    p_role = 'assistant'
    AND COALESCE(NULLIF(p_metadata->>'source', ''), NULLIF(p_source, ''), '') IN (
      'trial_onboarding',
    ...
    OR (
      p_role = 'assistant'
      AND (
        COALESCE(NULLIF(p_metadata->>'source', ''), '') LIKE 'trial_onboarding_day_%'
        OR COALESCE(NULLIF(p_metadata->>'source', ''), '') LIKE 'demo_reminder_%'
      )
    );
$$;
