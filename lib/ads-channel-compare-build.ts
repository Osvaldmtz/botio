...
function pickHigherWinner(metaValue: number, googleValue: number): ChannelMetricWinner {
...
function pickLowerCpaWinner(metaCpa: number | null, googleCpa: number | null): ChannelMetricWinner {
...
export async function buildChannelCompareResponse(): Promise<ChannelCompareResponse> {
  ...
  const metaCpaUsd =
    metaTotals && metaTotals.conversations > 0 ? metaSpendUsd / metaTotals.conversations : null;
  const googleCpaUsd =
    googleTotals && googleTotals.conversions > 0 ? googleSpendUsd / googleTotals.conversions : null;
