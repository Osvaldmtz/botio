...
import { runObjectionFollowupCron } from '@/lib/objection-followup-cron';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const supabase = createAdminClient();
    const summary = await runObjectionFollowupCron({ supabase, creds });
    return Response.json(summary);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error('[objection-followup] cron failed', error);
