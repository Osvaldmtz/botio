...
import { runDemoRemindersCron } from '@/lib/demo-reminders-cron';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const supabase = createAdminClient();
    const summary = await runDemoRemindersCron({ supabase, creds });
    return Response.json(summary);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error('[demo-reminders] cron failed', error);
